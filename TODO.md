...
_Last updated: 2026-07-24 — 0.3.9: ML accuracy push (XGBoost winner + corpus 1111→1275, +4% accuracy). 0.3.8: `ARCHITECTURE.md` refresh (header, §5.2, §8 status, new §13/§14). 0.3.7: `train.py` tests (50 cases, 0% → 95.2% cov; fixed `make_regressor` import bug). 0.3.6: `pip-audit` in CI (workflow blocks on prod CVE, 0 vulns in current tree). 0.3.5: `~=` pins for 9 prod deps (3-segment, stay-within-minor). 0.3.4: domain exception hierarchy (15 classes, 22 narrowed catches, 310 tests). 0.3.3: audit P0 fixes (+74 tests, 276 total, `business/app.py` 100% cov). 0.3.2: SSE auth fix (local-only `/api/stream/*` bypass). 0.3.0: Multikill classifier landed (degenerated on pro corpus); 0.2.2: calibration plumbing landed (empirical run showed overfit). 0.1.1: rate limit + SSE live updates._
