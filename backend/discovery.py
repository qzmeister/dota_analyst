...
def scrape_dltv_matches() -> List[Dict]:
    ...
    try:
        ...
        print(f"[discovery] dltv scrape failed: {exc}")
        return []
...
if STEAM_API_KEY:
    print(f"[discovery] Steam API key loaded ({STEAM_API_KEY[:6]}...{STEAM_API_KEY[-4:]})")
else:
    print("[discovery] Steam API key NOT configured — Steam live-discovery disabled")
...
def fetch_steam_live() -> List[Dict]:
    ...
    try:
        ...
        print(f"[discovery] steam fetch failed: {exc}")
        return []
            ...
            if stage == "live" and mid:
                ...
                if lj:
                    try:
                        ...
                        print(f"[discovery] synth failed for {mid}: {exc}")
                ...
                if not series and m.get("_steam_raw"):
                    try:
                        ...
                        print(f"[discovery] steam-synth failed for {mid}: {exc}")
                if series:
